...
const Quiz: React.FC = () => {
  ...
  const handleOptionClick = async (index: number) => {
    ...
    // Get simple AI feedback/explanation reinforcement
    const explanation = await generateQuizExplanation(currentQuestion.text, currentQuestion.options[index]);
      ...
      {selectedOption !== null && (
        <div className="absolute bottom-0 left-0 right-0 bg-gray-900/95 text-white p-6 z-20 animate-slide-up backdrop-blur-sm border-t-4 border-white/20">
          <div className="max-w-4xl mx-auto">
            ...
            {aiExplanation && (
                 <div className="bg-purple-800/50 p-3 rounded-lg border border-purple-500 text-sm animate-fade-in">
                    <span className="font-bold text-purple-300">🤖 Tutor AI: </span>
                    {aiExplanation}
                 </div>
            )}
          </div>
